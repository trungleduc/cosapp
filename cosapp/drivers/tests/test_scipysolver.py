...
def test_ScipySolverHybr_init():
    s = Multiply2("MyMult")
    d = s.add_driver(NonLinearSolver("run_drivers", method=NonLinearMethods.POWELL))
...
def test_ScipySolverHybr_get_residues():
    ...
    d._precompute()
    assert set(d.problem.residues) == {(name + "[p_out.x == 100.]")}
    assert d.problem.residues[
        name + "[p_out.x == 100.]"
    ].value == Residue.evaluate_residue(s.p_out.x, 100.0)


def test_ScipySolverHybr_fresidues():
    s = Multiply2("MyMult")
    d = s.add_driver(NonLinearSolver("run_drivers", method=NonLinearMethods.POWELL))
...
def test_ScipySolverHybr_singleptHybr(set_master_system):
    ...
    d.children["run"].design.add_unknown("K1").add_equation("p_out.x == 100.")
    s.call_setup_run()
    d.run_once()
    ...
    d = s.add_driver(NonLinearSolver("run_drivers", method=NonLinearMethods.POWELL))
    d.add_child(RunSingleCase("run"))
    d.children["run"].design.add_unknown(["K1", "K2"]).add_equation(
        ["p_out.x == 100.", "K2 == K1"]
    )
    s.K1 = 1.0
    s.call_setup_run()
...
def test_ScipySolverBroyden1_init():
    s = Multiply2("MyMult")
    d = s.add_driver(
        NonLinearSolver("run_drivers", method=NonLinearMethods.BROYDEN_GOOD)
    )
    assert len(d.children) == 1
...
def test_ScipySolverBroyden1_fresidues():
    s = Multiply2("MyMult")
    d = s.add_driver(
        NonLinearSolver("run_drivers", method=NonLinearMethods.BROYDEN_GOOD)
    )
    ...
    for idx, iterative in enumerate(d.problem.unknowns.values()):
        self.assertEqual(iterative.value, init[idx])
