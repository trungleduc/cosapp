...
class TestKrigingSurrogate(unittest.TestCase):
    ...
    def test_1d_predictor(self):
        ...
        self.assertTrue(np.allclose(mu, [[branin_1d(new_x)]], rtol=1e-1))
        self.assertTrue(np.allclose(sigma, [[0.07101449]], rtol=1e-2))
    ...
    def test_1d_ill_conditioned(self):
        ...
        self.assertTrue(np.allclose(mu, [[np.sin(0.5)]], rtol=1e-5))
    ...
    def test_vector_input(self):
        ...
        for x0, y0 in zip(x, y):
            mu, sigma = surrogate.predict(x0)
            self.assertTrue(np.allclose(mu, [y0], rtol=1e-9, atol=1e-12))
            self.assertTrue(np.allclose(sigma, [[0]], rtol=1e-6))
    ...
    def test_vector_output(self):
        ...
        for x0, y0 in zip(x, y):
            mu, sigma = surrogate.predict(x0)
            self.assertTrue(np.allclose(mu, [y0], rtol=1e-9))
            self.assertTrue(np.allclose(sigma, [[0, 0]], atol=1e-6))
    ...
    def test_scalar_derivs(self):
        ...
        self.assertTrue(np.allclose(jac[0][0], [1.], rtol=5e-3))
    ...
    def test_vector_derivs(self):
        ...
        self.assertTrue(np.allclose(jac, np.array([[1, 1], [1, -1], [1, 2]]), rtol=5e-4))
