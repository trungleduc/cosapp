...
function SetupLegend(d3, d3ContentDiv) {
    ...
    //COLORS
    {
        var text = ["System", "Port", "Calculated Variable", "Frozen Variable or Unconnected Input", "Collapsed", "Connection"];
        var colors = [GROUP_COLOR, COMPONENT_COLOR, UNKNOWN_EXPLICIT_COLOR, UNCONNECTED_PARAM_COLOR, COLLAPSED_COLOR, CONNECTION_COLOR];
        if (showParams) {
            text.splice(2, 0, "Connected Input");
            colors.splice(2, 0, PARAM_COLOR);
            text.splice(3, 0, "Unfrozen Variable or Unconnected Port")
            colors.splice(3, 0, UNKNOWN_IMPLICIT_COLOR)
        }
        for (var i = 0; i < text.length; ++i) {
    ...
    //N2 SYMBOLS
    {
        ...
        var shapeFunctions = [DrawVector, DrawScalar, DrawGroup];
        for (var i = 0; i < text.length; ++i) {
